...
$q_k(\zb^{(k)}) = p_k(\zb^{(k)} \mid X^{(k)})$, minimizing the right hand side of the equation with respect to $q_0$ leads to the barycenter problem as $\mathbb{E}_{ \zb^{(k)}\sim q_k}\left[\log  p_k (X^{(k)} \mid \zb^{(k)})\right]$ is a constant with respect to $q_0$.
