...
tags: Random_Partition High-dimensional_Bayesian_clustering Consensus_Monte_Carlo Optimal_Transport Variational_Inference
categories: blog
chart:
  plotly: true
---
...
In this post, we break down a new framework called [**Vertical Consensus Inference (VCI)**](https://arxiv.org/pdf/2603.27864) that tackles this issue using ideas from:
...
$$
\mathbf{z}^{(k)} \mid \mathbf{z}
\propto \exp\big(-\zeta_k \, c(\mathbf{z}^{(k)}, \mathbf{z})\big)
$$

$$
X^{(k)} \mid \mathbf{z}^{(k)} \sim p_k(X^{(k)} \mid \mathbf{z}^{(k)})
$$

So the joint distribution is:

$$
p(\mathbf{z}, \mathbf{z}^{(1:K)}, X)
= p(\mathbf{z}) \prod_{k=1}^K
p(\mathbf{z}^{(k)} \mid \mathbf{z}) \;
p_k(X^{(k)} \mid \mathbf{z}^{(k)})
$$
...
Fix $q_k$ as the shard posteriors, 
$q_k(\mathbf{z}^{(k)}) = p_k(\mathbf{z}^{(k)} \mid X^{(k)})$, minimizing the right hand side of the equation with respect to $q_0$ leads to the barycenter problem as $\mathbb{E}_{ \mathbf{z}^{(k)}\sim q_k}\left[\log  p_k (X^{(k)} \mid \mathbf{z}^{(k)})\right]$ is a constant with respect to $q_0$.
